...
- **端末内翻訳 + AI要約** (Apple Foundation Models、iOS/macOS 26+):
  ...
  が表示されますが、**翻訳は「翻訳」ボタンを押したときだけ実行されます
  （自動翻訳は既定オフ）**。訳文を表示中はワンタップで原文に戻せます。
  HTML メールは表・画像・罫線などのレイアウトを保ったまま本文の文字
  だけが翻訳されます（プレーンテキストのメールのみ、段落を長押しする
  とその段落だけ原文を確認できます）。「AI要約」バーは言語を問わずどの
  メッセージにも表示され、タップで端末内生成の短い要約を表示します。
  「英語で返信を下書き」で英訳前提のまま返信作成に入れることもできます。
  自動翻訳は設定でオンに戻せます。データは一切端末の外に出ません —
  エンジンの設計と唯一知られている Simulator 固有の制限は
  [`docs/translation.md`](docs/translation.md) を参照してください。
...
- メッセージごとの翻訳バー。英文メールを開くと表示されますが、翻訳は
  「翻訳」ボタンを押したときだけ実行されます（自動翻訳は既定オフ —
  設定でオンに戻せます）。訳文表示中はセグメントコントロールで原文に
  切り替えられます。プレーンテキストのメールは段落を長押しすればその
  段落だけ原文を確認でき、HTML メールは表・画像・罫線などのレイアウト
  を保ったまま本文の文字だけが翻訳されます。

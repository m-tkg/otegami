...
- **端末内翻訳 + AI要約** (Apple Foundation Models、iOS/macOS 26+):
  翻訳ボタンは本文の読み込みが終わっていれば常に押せます（言語判定に
  よる表示/グレーアウトはしていません — 英文メールなのにボタンが出ない
  という実機報告が繰り返されたため撤去済み）。**翻訳は「翻訳」ボタンを
  押したときだけ実行されます（自動翻訳は既定オフ、オンにしても実際に
  英語だと確信を持てたメールでしか自動発火しません）**。訳文を表示中は
  ワンタップで原文に戻せます。HTML メールは表・画像・罫線などのレイアウト
  を保ったまま本文の文字だけが翻訳されます（プレーンテキストのメールの
  み、段落を長押しするとその段落だけ原文を確認できます）。「AI要約」
  バーは言語を問わずどのメッセージにも表示され、タップで端末内生成の
  短い要約を表示します。自動翻訳は設定でオンに戻せます。データは一切
  端末の外に出ません — エンジンの設計と唯一知られている Simulator 固有の
  制限は [`docs/translation.md`](docs/translation.md) を参照してください。
...
- **HTML メール**: サンドボックス化された `WKWebView` で描画（JavaScript
  ...
  無害化されることを確認）。幅600-800px級の固定幅テーブルで組まれた
  マーケティング/通知メール（銀行の入出金通知など）も fit-to-width で
  画面幅に収めて表示し、右端がクリップしたり文字が巨大に見えたりしません。
  詳細画面には控えめな「HTML」バッジが付き、ワンタップでテキスト表示に
  切り替え可能（設定の「常にテキストで表示」で既定を変更することも可能）。
  本文が完全に空のメールは、空欄のままではなく
...
- **添付ファイル**: 送受信・QuickLook プレビュー・インライン `cid:` 画像
  ...
  にも対応しています。
- **カレンダー招待メール**: Google カレンダー形式の招待メール
  (`text/calendar; method=REQUEST` パート) を検出すると、タイトル・日時
  （端末のタイムゾーンに変換）・場所・主催者を示す招待カードを表示し、
  「承諾」「辞退」「未定」ボタンで出欠を選べます。ボタンをタップすると
  標準の iTIP `METHOD:REPLY` を主催者へ返信するだけなので、カレンダー
  API へのアクセスや OAuth スコープなしで Google カレンダー側の出欠が
  更新されます。詳細は [docs/calendar-invites.md](docs/calendar-invites.md)
  参照。
...
- **mailto: リンク・デフォルトのメールアプリ**: otegami は両プラット
  フォームで `mailto` スキーム (`CFBundleURLTypes`) を登録しており、
  `mailto:` リンクをタップ/クリックすると to/cc/bcc/件名/本文
  (RFC 6068 準拠でデコード) をプリフィルした作成画面が開きます。macOS
  はこれだけでデフォルトのメールアプリの候補になれます (Mail.app の
  「設定」→「一般」、またはシステム設定の「デスクトップと Dock」)。iOS
  で実際にシステム全体のデフォルトになるには、Apple の
  `com.apple.developer.mail-client` entitlement が別途必要で、これは
  既定オフのビルド時フラグです (フラグ無しでも通常どおりビルド・
  アーカイブできます) — 詳細は
...
- **一覧表示**: カード状の行（輪郭線は描かず、角丸+面の色だけで区切る
  ...
  のフラット表示）、差出人ごとのアバター（一覧・詳細どちらにも表示可能。
  優先順位は (1) この端末の連絡先 (Contacts framework、オンデバイスのみ・
  外部送信なし) に一致する写真、(2) Gravatar (gravatar.com、差出人アドレ
  スの SHA-256 ハッシュを送信して画像を取得)、(3) 差出人ドメインの
  favicon による企業ロゴ (Gmail/iCloud 等の主要フリーメールドメインは
  対象外)、(4) イニシャル + アカウント色。外部通信を伴う各情報源は設定
  →「メール一覧」で個別にオフにできる）、本文プレビューの行数設定に
  対応。スレッドを開いた時の折りたたみ
  行にも同じアイコン/プレビューを表示しますが、トップの一覧と見分けが
  つくよう控えめなトーンで区別しています。
- **スレッドの読み方はプラットフォームごとに最適化**: iOS では2通以上の
  スレッドをタップするとまずスレッド選択画面 (一覧と同じアイコン/
  プレビュー/時刻の行が並ぶ) が開き、1通選ぶとそのメッセージ単体の本文
  画面に直接遷移します — アコーディオンやスタックは画面上に一切残り
  ません。1通しかないスレッドは選択画面自体をスキップして直接本文へ。
  macOS は画面の広い3ペイン構成のまま、従来どおりの厳密なアコーディオン
  を維持します: 常に1通だけ展開され、別のメッセージのヘッダをタップする
  とそれが開き他は畳まれます。展開中のメッセージはアクセントカラーの
  縦罫線 (一覧のアカウント色罫線と同じ見た目) で視覚的に強調されます。
  いずれの場合も、本文画面下部のツールバー（返信・転送・検索・情報）は
  常に現在表示中/展開中のメッセージを対象にします。
...
アカウント絞り込みチップ＋「未読のみ表示」トグルの常設1画面＋左上の
ハンバーガーメニュー（フォルダ切替と設定 — 既定は受信トレイ/アーカイブ/
送信済み/下書き等 role ごとにセクションを作り各アカウントをその中に
並べる「カテゴリ優先」表示＋role ごとの「すべての◯◯」横断ビュー行、
セグメントコントロールで従来の「アカウントごと」表示にも切替可能）＋
左下フローティングの検索
ボタン（アカウント絞り込みチップ・`from:`/`to:`/`cc:`/`subject:` 検索
演算子・検索履歴を備えた検索画面）＋pull-to-refresh による再同期＋
...
- メッセージごとの翻訳ボタン。本文の読み込みが終わっていれば常に押せ
  ます（言語判定による表示/グレーアウトはしていません — 英文メールなの
  にボタンが出ない/押せないという実機報告が繰り返されたため撤去済み）。
  翻訳は「翻訳」ボタンを押したときだけ実行されます（自動翻訳は既定オフ
  — 設定でオンに戻せますが、実際に英語だと確信を持てたメールでしか自動
  発火しません）。訳文表示中はセグメントコントロールで原文に切り替え
  られます。プレーンテキストのメールは段落を長押しすればその段落だけ
  原文を確認でき、HTML メールは表・画像・罫線などのレイアウトを保った
  まま本文の文字だけが翻訳されます。

...
- **オフラインファースト**: メッセージ・スレッド・フラグの変更はすべて
  ...
  自動的にリプレイされます。
- **端末内翻訳** (Apple Foundation Models、iOS/macOS 26+): 英文メールには
  インラインの翻訳バーが表示され、既定では日本語訳を表示（原文への切替は
  ワンタップ）。段落を長押しすればその段落だけ原文を確認できます。
  「英語で返信を下書き」で英訳前提のまま返信作成に入れるほか、作成画面
  自体にも「英語に翻訳して送る」トグルがあり、送信前に日本語の下書きを
  その場で英訳（結果を見てから送信、裏で黙って翻訳されることはありません）
  できます。自動翻訳は設定でオフにできます。データは一切端末の外に出ません
  — エンジンの設計と唯一知られている Simulator 固有の制限は
  [`docs/translation.md`](docs/translation.md) を参照してください。翻訳
  自体は実機相当のオンデバイスモデルに対して毎回2〜5秒で成功することを
  確認済みです。
...
- **アカウント絞り込みチップつき統合受信トレイ**: 全アカウントのメールを
  ...
  アカウント色のアクセントと、メールボックス単位/統合の未読数バッジ。
...
- **HTML メール**: サンドボックス化された `WKWebView` で描画（JavaScript
  は無効、外部画像はワンタップの「画像を表示」バナーの裏でブロック、
  インラインの `cid:` 画像はローカルで解決）。詳細画面には控えめな
  「HTML」バッジが付き、ワンタップでテキスト表示に切り替え可能（設定の
  「常にテキストで表示」で既定を変更することも可能）。本文が完全に空の
  メールは、空欄のままではなく薄い文字で「本文なし」と表示。
...
- **作成/返信**: 誤送信防止のための差出人選択必須、プレーンテキストでの
  ...
  確認、IMAP 経由の下書き双方向同期。iOS では送信も消失防止・取り消し
...
- **一覧表示**: スレッド表示のオン/オフ（オフにすると1メール1行のフラット表示）、
  差出人のイニシャル + アカウント色によるアイコン（一覧・詳細どちらにも
  表示可能、外部サービスへの問い合わせは一切なし）、本文プレビューの
  行数設定に対応。
...
- 「英語で返信を下書き」と、作成画面側の「英語に翻訳して送る」トグル
  ——後者は自分が書いた下書きをその場で英訳します（送信前に結果を確認・
  編集できる設計で、裏で黙って翻訳されることはありません）。

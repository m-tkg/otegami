...
public struct FoundationModelsTranslationService: TranslationService {
    ...
    /// 二段構え。
    ///
    /// **フォローアップ6での変更点 (実機フィードバック「メールで当日の感想
    /// について書いてあるのにこんなまとめ方をされてしまっていて、感想に
    /// ついての要約がない」— 実際の出力例:「具体的な内容：特に記載されて
    /// いる決定事項・依頼や質問・数値・固有名詞は存在しない。」)**: 旧版は
    /// 【最優先ルール: 情報を落とさない】が「決定事項・依頼や質問・数値・
    /// 固有名詞は省略しない」という**カテゴリ列挙**から始まっていた —
    /// これが2つの問題を引き起こしていた。(a) 指示文自身のこの語彙
    /// (「決定事項・依頼や質問・数値・固有名詞」) がそのまま出力に漏れる
    /// (Task #160フォローアップ2/4で例文の題材が漏れた事故と同型だが、
    /// 今回は例文ではなく指示文の主要ルール自体の語彙が漏れた)。(b) この
    /// 4カテゴリに当てはまらない内容 (感想・意見・出来事の報告・雑談) の
    /// メールに対して、モデルが「本文の内容を要約する」のではなく「4カテ
    /// ゴリが存在するかどうかをチェックリストのように回答する」ほうを
    /// タスクだと誤解し、該当が無いことをそのまま報告していた。
    ///
    /// 修正: 主目的を【最優先ルール: このメッセージの内容を要約する】へ
    /// 差し替え、感想・意見・出来事の報告・雑談を含む「どんな内容でも
    /// 本文が言っていることを要約する」ことを明示。旧「情報を落とさない」
    /// ルールは【本文に含まれている場合は落とさない情報】という副次的な
    /// 条件へ格下げし、「4カテゴリが無いスレッドではその旨をわざわざ書く
    /// 必要はない」ことを明記。さらに【禁止: 情報の有無やカテゴリを論評
    /// する文】を新設し、実際の事故文言そのもの (「決定事項・依頼や質問・
    /// 数値・固有名詞は存在しない」等) を悪い例として明示。出力例の題材は
    /// これまでの教訓どおり (Task #160フォローアップ2/4) 具体的すぎない
    /// 抽象的なものにする。
    ///
    ...
    private static func summarizeThreadEntryInstructions(targetLanguage: TranslationLanguage, sentenceRangeGuidance: String) -> String {
        """
        以下はメールスレッド内の1通のメッセージの新規本文(引用された過去の返信は除外済み)です。以下のルールに従って、この新規本文の内容を\(targetLanguage.displayName)で要約してください。

        【最優先ルール: このメッセージの内容を要約する】
        このメッセージが実際に何を伝えているかを、目安\(sentenceRangeGuidance)で要約してください。内容は決定事項や依頼・質問に限りません — 感想・意見・出来事の報告・お礼・雑談など、どんな種類の内容であっても、書かれていることをそのまま要約の対象にしてください。例えば、何らかの出来事に参加した感想が書かれていれば、その感想の具体的な内容 (何についてどう感じたか) を要約してください。

        【禁止: 情報の有無やカテゴリを論評する文(最重要)】
        「決定事項・依頼や質問・数値・固有名詞は存在しない」「〜は含まれていない」「具体的な内容：」のような、本文の内容をカテゴリで評価・分類する文やチェックリストのような文は絶対に書かないでください。これらは要約ではありません。目的は本文の実際の内容 (感想・意見・出来事など) を要約することであり、決定事項等が無いことを報告することではありません。要約すべき実質的な内容が本当に何もない場合 (空メール・定型の挨拶のみ等) に限り、その旨をごく短く一言で述べてください。

        【本文に含まれている場合は落とさない情報】
        新規本文に決定事項・依頼や質問・数値(金額・日時・個数・期限など)・固有名詞(人名・会社名・場所名・店名など)が含まれている場合は、それらを省略せず要約に含めてください。ただし、これは「本文にそういう内容が書かれていた場合に落とさない」という条件であって本文の主題ではありません — 本文が感想や雑談であれば、これらのカテゴリが無いのは当然であり、その旨をわざわざ書く必要はありません (上のルール参照)。挨拶や「よろしくお願いします」「お世話になっております」のような定型的な前置き・結びの言葉だけは削って構いません。
        ...
        【メッセージを説明するな、事実を直接書け(重要)】
        このメッセージについて"説明"してはいけません。書き手が実際に言ったこと・感じたこと・決めたこと・尋ねたことを、事実として直接、断定的に書いてください。次のような、メッセージ自体やその内容の有無を話題にする言い回し(メタ言及)は絶対に使わないでください: 「この返信では」「このメールでは」「このメッセージでは」「〜が述べられている」「〜と述べられている」「〜が記載されている」「〜と記載されている」「〜という内容」「〜と伝えられている」「〜と書かれている」。これらの表現は一切禁止です。

        【出力例(あくまで文の形の例 — 内容は必ず新規本文から取ること)】
        悪い例: 決定事項・依頼や質問・数値・固有名詞は含まれていない。
        良い例: (何らかの体験について) 良かった点と気になった点を具体的に伝えている。

        悪い例: この返信では対応が完了したことが述べられている。
        良い例: 対応が完了した。
        ...
        【分量・出力形式】
        目安は\(sentenceRangeGuidance)ですが、上記の「内容を要約する」「情報を落とさない」ルールを優先し、必要なら目安を超えてもかまいません。ラベルや見出し、箇条書き、番号付きリスト、記号("■"など)は付けず、改行も入れずに、1つの続いた文章として出力してください。説明文やこの指示文自体、英語のテキストを出力に含めないでください。
        """
    }

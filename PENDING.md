...
## Task #160: スレッド要約のmapをメッセージ単位に固定・二重圧縮とメタ言及調を根治 — 実機確認
...
進捗表示 (「n/m 通目を要約中…」) は`ThreadDetailView`の生成中シートに
維持。詳細・実FoundationModels確認ログ (before/after 含む) は
`docs/translation.md`の「スレッド全体のAI要約: メッセージ単位のmap段への
改修 (Task #160)」「スレッド要約の二重圧縮を根治 (Task #160フォロー
アップ)」「スレッド要約のメタ言及調を除去 (Task #160フォローアップ2)」
の3節参照。`make test`/`make mac`/`make ios` green (全段階とも)。

**残っている実機確認**:
- スレッド要約の生成に今までよりかかる時間 (メッセージ数ぶん FM を
  実行するため) と、その間の「n/m 通目を要約中…」表示が実機で違和感
  なく見えるか — シミュレータでのスクリーンショット確認はしていない
  (Foundation Modelsのシミュレータ既知不調のため)。
- 実際のスレッド (架空フィクスチャでなく実データ) で、数値・固有名詞・
  決定事項が■経緯から抜け落ちなくなったか、かつ「この返信では〜」の
  ような説明調の言い回しが実際に消えたと感じられるか。実FM確認 (フォロー
  アップ2適用後) では3回とも金額・日時・固有名詞が対応するメッセージの
  行にきちんと残り、メタ言及・ハルシネーションともに無かったが、実データ
  はフィクスチャより長く複雑な文面が多く、同様の傾向になるとは限らない。

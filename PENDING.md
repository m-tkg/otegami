...
**実装状況**: ユーザー要望「iCloud でアカウントの設定以外も全て同期して
欲しい」を受け、表示・操作設定 (既存の`settings.v2`) の対象を拡張し、
署名テンプレート・作成テンプレート本体は新設の`TemplateCloudSyncEngine`
(`templates.v1`) で同期するようにした。macOS を`settings.v2`の同期から
除外していた以前の方針 (2026-07-29) も、今回の要望どおりユーザー自身が
明示的に反転させたため撤回し、macOS も iOS/iPadOS と同じ条件で同期する。
棚卸し結果・分類の根拠・競合解決方針の詳細は`docs/icloud-sync.md`
「Task #186: 設定全般の同期範囲の棚卸しと拡張」節参照。

`make test`/`make mac`/`make ios`/`make check-localization`全て緑。
新規ユニットテスト (`TemplateReconcilerTests`/`TemplateCloudSyncEngineTests`、
`packages/OtegamiKit/Tests/AccountCloudSyncTests/`) で純粋な同期ロジック
(push/pull/last-writer-wins競合/tombstone削除/重複検出) を検証済み。

**未確認 (実機・実 iCloud での確認が必要 — シミュレータでは実 iCloud KVS
の挙動を安定して検証できないため、`docs/icloud-sync.md`の実機汚染
インシデント節参照)**:

1. **macOS が設定を同期するようになったことの確認**: Mac で表示設定
   (スレッド表示・スワイプ割り当てなど) を変更し、iPhone/iPad 側に
   反映されること。逆方向 (iPhone/iPad → Mac) も。
2. **今回追加した4設定の同期**: 「メール内リンクを開くブラウザ」
   「削除・アーカイブ後の挙動」「送信取り消しの猶予秒数」「デフォルト
   のアカウント」がそれぞれ2端末間で反映されること。
3. **署名テンプレートの同期**: 一方の端末で署名テンプレートを新規作成
   → もう一方の端末で (アプリ起動 or フォアグラウンド復帰後に) 同じ
   署名が現れること。名前・本文・対象アカウントを編集 → 反映されること。
   削除 → もう一方でも消えること。
4. **作成テンプレート (C8) の同期**: 3と同様の一連の確認。
5. **アカウントごとの最後に選んだ署名**: 一方の端末で Composer の署名
   ピッカーから選び直す → もう一方の端末の Composer でも (そのアカウント
   を選んだときに) 同じ署名が事前選択されること。
6. **競合の実地確認**: 2端末をほぼ同時にオフラインにしてそれぞれ別の
   署名を編集 → オンラインに戻したとき両方の変更が残ること (`docs
   /icloud-sync.md`が説明する「項目ごとのlast-writer-wins」の実地確認)。
7. **同じ署名を両端末でほぼ同時に編集した場合**: 最終的にどちらか一方の
   編集内容に統一され、アプリがクラッシュしたり壊れた表示にならない
   こと (フィールド単位のマージはしない設計を実地で確認)。
...
## Task #187: Yahoo! JAPAN 断続的認証失敗の再試行間隔是正 — 実機確認が未検証

詳細・根拠 (実測したサーバー応答文字列・出典 URL 含む) は
`docs/qa-findings.md`の Task #187 節参照。要点: リレー
(`server/otegami-relay-go`) とアプリ (`AccountSyncer.runIdleLoop`) の
両方で、認証失敗時の再試行間隔を「一度でも成功実績のある資格情報」に
限って30分 (リレーは失敗のたびに倍々、上限6時間) へ延ばし、リレー側は
成功実績があれば恒久停止しないようにした。「接続テスト」の文言も、
成功実績のあるアカウントでは「パスワードを確認してください」と断定
しないよう変更した。`make test`/`make mac`/`make ios`/
`make check-localization`すべて green、Go側`go build`/`go vet`/
`go test`も green。**シミュレータの Fake IMAP サーバーでのユニット
テストのみ検証済みで、実 Yahoo! JAPAN アカウントでの確認は未実施**
(実機作業のため)。以下、実機で確認してほしい点:

1. **ロック再発の実地確認**: 実際にこの断続的失敗が起きている Yahoo!
   JAPAN アカウントで、修正版アプリ + リレーを一定期間 (半日〜1日程度)
   動かし、以前より認証失敗の頻度・持続時間が改善するか (「一度ロック
   すると手動で気づくまで直らない」から「30分〜数時間待てば自然に
   復帰する」へ変わっているか)。
2. **リレーの`GET /v1/watches`表示**: 認証失敗中でも`status`が
   `stopped`にならず`active`のままであること (アプリの
   `PushNotificationSettingsView`の該当行が「登録済み」のままになる
   はず — 「停止中で気づかない」というトレードオフを許容するか、
   ユーザー視点でも妥当か合わせて確認)。
3. **「接続テスト」文言の実地確認**: 認証失敗中の既存アカウントを
   `AccountEditView`で開き「接続テスト」を押した時、実際に
   「この資格情報は過去に接続に成功しているため…」の文言に切り替わって
   いること (`hasAccountEverSyncedSuccessfully()`が実データで正しく
   `true`と判定できているか — 開発環境の`FakeIMAPSession`でしか
   確認できていない)。
4. **30分/6時間という値の体感確認**: 実際のロック解除タイミングと
   比べて30分が早すぎる (まだロック中で無駄打ちする) か遅すぎる
   (もっと早く復帰できるのに待たせている) か、体感で分かれば
   `docs/qa-findings.md`のTask #187節に追記して値を見直す判断材料に
   する。

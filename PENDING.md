...
### Task #191 (ハンバーガーメニューの行を圧縮): アコーディオン開閉アニメーションの実機確認が未実施

「各項目の高さが高すぎてスカスカ」への対応。行/セクション見出しの実際の
高さをピクセル計測で切り分けたところ、原因は`List`の`Section`境界に
付く固定余白 (SwiftUI/UIKit 側に公開APIなし) と判明したため、折りたたみ
中はカテゴリ/アカウントの見出しを`Section`ではなくただの行として描画する
方式に変更した (展開中だけ`Section`に包み直す)。`scripts/verify-screen.sh
menu`/`menu-expanded`の静的スクリーンショットでは、折りたたみ・展開とも
見た目・タップ領域 (44pt以上) を確認済み。詳細: `docs/design-system.md`
「Task #191」節。

未検証:
- **タップ時のアコーディオン開閉アニメーション**: 折りたたみ中は
  `Section`ではなくただの行として描画するよう変更したため、タップして
  展開/折りたたみするたびに見出し行の「入れ物」自体が`Section`⇄ただの行
  に切り替わる。`withAnimation(.default)`で包んではいるが、静的スクリー
  ンショットのペアでは実際のアニメーション (なめらかに開閉するか、
  見出し行が一瞬ちらつく/ジャンプしないか) は確認できていない。実機/
  シミュレータでハンバーガーメニューのカテゴリ・アカウント見出しを
  何度かタップし、違和感のない開閉になっているか確認してほしい。
- **他のセクションが自動で畳まれる際の見た目**: アコーディオン挙動
  (1つ開くと他が閉じる) 自体は既存ロジック (`toggleCategoryCollapsed`/
  `toggleAccountCollapsed`) を変更していないが、`Section`⇄行の切り替えが
  複数箇所同時に起きるため、同上の確認と合わせて見てほしい。

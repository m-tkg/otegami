...
## Task #160: スレッド要約のmapをメッセージ単位に固定・二重圧縮とメタ言及調を根治・仕上げパスで簡潔化 — 実機確認
...
の4節参照。`make test`/`make mac`/`make ios` green (全段階とも)。

**残っている実機確認**:
- スレッド要約の生成に今までよりかかる時間 (メッセージ数ぶん FM を
  実行し、4通超のスレッドは仕上げパスでもう1回実行するため) と、その間の
  「n/m 通目を要約中…」「仕上げ中…」表示が実機で違和感なく見えるか —
  シミュレータでのスクリーンショット確認はしていない (Foundation Models
  のシミュレータ既知不調のため)。
- 実際のスレッド (架空フィクスチャでなく実データ) で、数値・固有名詞・
  決定事項が■経緯から抜け落ちなくなったか、「この返信では〜」のような
  説明調の言い回しが消えたか、かつ往復の多いスレッドで■経緯が以前より
  簡潔に感じられるか。実FM確認 (フォローアップ3適用後) では3回とも6行→
  4〜5行に減り、金額・日時・固有名詞・帰属が正しく保たれ、メタ言及・
  ハルシネーションともに無かったが、実データはフィクスチャより長く複雑な
  文面が多く、同様の傾向になるとは限らない。
- (軽微・スコープ外) ■現状パートが箇条書き+空行混じりの形式で返る
  ことが実FM確認で時々あった — 表示は壊れないが、追加の報告があれば
  `currentStatusInstructions`の調整を検討する。
- (軽微・スコープ外・新規) 仕上げパスの■経緯が、指示で明示的に禁止した
  「・」箇条書き記号を付けて返ることが3回中2回あった — `SummaryText`の

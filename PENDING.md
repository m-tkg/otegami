...
## Task #160: スレッド要約のmapをメッセージ単位に固定 — 実機確認

**実装状況**: `TranslationService.summarizeThread`のmap段を、文字数
ベースの`TranslationChunker`分割からメッセージ単位に固定し、短い
スレッドでも必ず全メッセージが`summarizePlain`を1回ずつ通るように
した。進捗表示 (「n/m 通目を要約中…」) を`ThreadDetailView`の生成中
シートに追加。詳細・実FoundationModels確認ログ (3回実行の実出力) は
`docs/translation.md`「スレッド全体のAI要約: メッセージ単位のmap段への
改修 (Task #160)」節参照。`make test`/`make mac`/`make ios` green。
...
- 実際のスレッド (架空フィクスチャでなく実データ) で■経緯が「短すぎる」
  という元の報告が解消したと感じられるか。実FM確認では3回中2回が改善を
  明確に示したが、1回はやや簡潔め・軽微な言い回しの重複が出た — 実機の
  実データでも同様の揺らぎがあり得る。

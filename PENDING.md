...
## Task #160: スレッド要約のmapをメッセージ単位に固定・二重圧縮とメタ言及調を根治・仕上げパスで簡潔化・■現状のハルシネーション根治 — 実機確認

**実装状況 (5段階、最終段は最優先ユーザー指示で緊急対応)**:
1. map段を、文字数ベースの`TranslationChunker`分割からメッセージ単位に
   固定 (短いスレッドでも必ず全メッセージがモデルを1回ずつ通る)。
2. フォローアップ: それでも「内容がほとんど抜け落ちている」という続報を
   受け、map段を「圧縮(`summarizePlain`)」から「事実抽出
   (新設`summarizeThreadEntry`— 決定事項・数値・日付・固有名詞を落と
   さない指示)」へ置き換え、かつ■経緯をモデルで再圧縮するのをやめて
   アプリ側で箇条書きに組み立てるよう再設計 (■現状だけ引き続きモデル
   1回)。二重圧縮 (map+reduceの両方が「短くする」方向だったこと) が
   実機報告の根本原因だったため。
3. フォローアップ2: 「『この返信では〜が述べられている』のような説明調
   (メタ言及)の口調が気になる」という続報を受け、`summarizeThreadEntry`
   の指示文から「差出人・宛先名を常に『この返信』に置き換える」ルールを
   撤去 (これがメタ言及の原因だった) し、禁止表現の明示 + 悪い例/良い例
   + アプリ側の保険的サニタイズ (`ThreadEntryMetaCommentaryStripper`、
   過剰除去防止のため自己参照オープナーで始まる文だけを対象にする設計)
   を追加。実FM確認1回目で指示文の例文自体 (偶然フィクスチャと同じ題材)
   が別メッセージの数値ハルシネーションを誘発したため、例文を差し替えて
   2回目で解消を確認。
4. フォローアップ3: ユーザー要望「要約済みのものを再度読ませてさらに
   要約を挟ませてシンプルにする」を受け、map (`summarizeThreadEntry`) と
   reduce (`summarizeThreadDigest`、■現状) の間に任意の「仕上げ」パス
   `refineThreadEntries`を追加 — per-message抽出結果を1回読み、同じ話題
   への複数の応答をまとめて簡潔化した■経緯を生成する (帰属は保持)。
   3通以下はスキップ、失敗時は無加工列挙へフォールバック。進捗表示に
   「仕上げ中…」を追加 (`ThreadSummaryProgress`)。
5. フォローアップ4 (最優先・ユーザー本人の明示指示): 「■現状に全然関係
   ない話が出てきた」というハルシネーション報告を受け、根本原因
   (`currentStatusInstructions`の【出力例】がフォローアップ2/3と同型の
   「例文の題材が漏れる」バグを踏んでいた、かつ実FM確認フィクスチャと
   題材が一致していたため過去の確認で発見できなかった) を特定・修正。
   加えて多層防御: 接地ルールの明記、新設`ThreadDigestGroundingCheck`
   (数値・カタカナ語・ラテン文字語の入力照合) による検証 + 1回だけの
   再生成 + 機械的フォールバック (最後のメッセージの抽出内容)。実FM確認
   1周目で「短いスレッドで入力に無い後続アクションを作る」という別の
   軽微な残存問題を発見し、2周目で指示文をさらに調整して解消を確認。

進捗表示 (「n/m 通目を要約中…」「仕上げ中…」) は`ThreadDetailView`の
生成中シートに維持。詳細・実FoundationModels確認ログ (before/after 含む)
は`docs/translation.md`の「スレッド全体のAI要約: メッセージ単位のmap段
への改修 (Task #160)」「スレッド要約の二重圧縮を根治 (Task #160フォロー
アップ)」「スレッド要約のメタ言及調を除去 (Task #160フォローアップ2)」
「スレッド要約の『仕上げ』パスでさらに簡潔化 (Task #160フォローアップ3)」
「■現状のハルシネーションを多層防御で根治 (Task #160フォローアップ4、
最優先)」の5節参照。`make test`/`make mac`/`make ios` green (全段階とも)。

**残っている実機確認**:
- スレッド要約の生成に今までよりかかる時間 (メッセージ数ぶん FM を
  実行し、4通超のスレッドは仕上げパスでもう1回実行、■現状がグラウン
  ディング検査に落ちた場合はさらにもう1回実行するため) と、その間の
  「n/m 通目を要約中…」「仕上げ中…」表示が実機で違和感なく見えるか —
  シミュレータでのスクリーンショット確認はしていない (Foundation Models
  のシミュレータ既知不調のため)。
- 実際のスレッド (架空フィクスチャでなく実データ) で、■現状に無関係な
  話が出なくなったか、数値・固有名詞・決定事項が■経緯から抜け落ちなく
  なったか、「この返信では〜」のような説明調の言い回しが消えたか、かつ
  往復の多いスレッドで■経緯が以前より簡潔に感じられるか。実FM確認
  (フォローアップ4適用後、6メッセージ・短いスレッド・未決スレッド・
  通知メールのみスレッドの4種×3回=12回) では、いずれも無関係な話題の
  混入は無かったが、実データはフィクスチャより長く複雑な文面が多く、
  同様の傾向になるとは限らない。
- (軽微・スコープ外) ■現状パートが箇条書き+空行混じりの形式で返る
  ことが実FM確認で時々あった — 表示は壊れないが、追加の報告があれば
  `currentStatusInstructions`の調整を検討する。
- (軽微・スコープ外) 仕上げパスの■経緯が、指示で明示的に禁止した
  「・」箇条書き記号を付けて返ることが時々あった — `SummaryText`の
  表示は壊れない (実害なし) ため見送ったが、実機で見た目の違和感が
  報告されれば`refineThreadEntriesInstructions`の禁止表現をさらに強める。
- (軽微・スコープ外・新規) 通知メールのみのスレッドで、■現状の末尾に
  独立した「(未決)」行が付く体裁の癖が実FM確認で2回あった — 内容自体は
  入力に基づいており実害なし。

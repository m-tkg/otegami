...
## Task #160: スレッド要約のmapをメッセージ単位に固定・二重圧縮を根治 — 実機確認
...
`docs/translation.md`「スレッド全体のAI要約: メッセージ単位のmap段への
改修 (Task #160)」「スレッド要約の二重圧縮を根治 (Task #160フォロー
アップ)」の両節参照。`make test`/`make mac`/`make ios` green (両段階とも)。
...
- 実際のスレッド (架空フィクスチャでなく実データ) で、数値・固有名詞・
  決定事項が■経緯から抜け落ちなくなったと感じられるか。実FM確認 (二重
  圧縮の根治後) では3回とも金額・日時・固有名詞が対応するメッセージの
  行にきちんと残っていたが、実データはフィクスチャより長く複雑な文面が
  多く、同様の傾向になるとは限らない。

...
/// nothing to type here — the whole flow is one button that runs
/// `ASWebAuthenticationSession` (via `AppEnvironment.requestGmailAuthorization()`)
/// and, on success, saves the account itself
/// (`AppEnvironment.createGmailAccount(email:tokens:)`) using the address
/// Google reports rather than anything typed in this form.
///
...
struct GmailAccountSetupView: View {
    ...
    private func signIn() async {
        ...
        do {
            ...
            try await environment.createGmailAccount(email: email, tokens: tokens)
            dismiss()

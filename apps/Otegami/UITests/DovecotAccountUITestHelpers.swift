...
extension XCTestCase {
    ...
    /// 画面構造改修バッチ (Task #33, 1) → Task #136 (実機フィードバック
    /// 「アコーディオンに戻してほしい」) で revert 済み: tapping a thread row
    /// (any message count) always lands on `ThreadDetailView` directly again
    /// — the intermediate `ThreadSelectionView` this helper used to route
    /// through no longer exists. Kept as a thin wrapper around
    /// `threadDetail.scrollView`'s own `waitForExistence` (rather than
    /// inlining that call at every one of this helper's several call sites)
    /// so none of them needed touching when the selection screen went away
    /// — they all just keep asserting "did opening a thread work at all,"
    ...
    func waitForThreadDetailPossiblyThroughSelectionScreen(in app: XCUIApplication, timeout: TimeInterval = 20) -> Bool {
        app.scrollViews["threadDetail.scrollView"].waitForExistence(timeout: timeout)
    }

...
/// design-phase-3 verification (1j): the filter chip row (全部/添付/未読 —
/// `SearchFilterOption`; the「英語」chip that used to be part of this row
/// was retired in the 検索画面再構成 batch, Task #86) and 人/メール
/// sectioning added on top of M7's search baseline. Builds on whatever
/// account/seed state earlier phases (M7, the translation UITests) already
/// established in this simulator install — same "each phase its own
/// `-only-testing:` invocation" pattern as the rest of this suite
/// (`docs/verify.md`).

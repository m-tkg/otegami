...
- [ ] **スレッド要約 (■経緯/■現状) がメッセージ単位で丁寧になったか** —
      複数往復のあるスレッドを開き、ナビゲーションタイトル横の要約ボタンを
      タップして「n/m 通目を要約中…」という進捗表示が出ること、生成完了後
      の■経緯が各メッセージの流れをきちんと追えていること (元の実機報告
      「短すぎるし内容も少し変」が解消しているか) を確認する。

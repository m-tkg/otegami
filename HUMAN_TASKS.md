...
- [ ] **スレッド要約 (■経緯/■現状) の内容が具体的になったか (二重圧縮
      根治後)** — 複数往復のあるスレッド (できれば金額・日時・固有名詞
      など具体的な内容を含むもの) を開き、ナビゲーションタイトル横の要約
      ボタンをタップして「n/m 通目を要約中…」という進捗表示が出ること、
      生成完了後の■経緯が1メッセージ1行の箇条書きになっていて、各行に
      そのメッセージの具体的な内容 (数値・固有名詞・決定事項) がきちんと
      残っていること (「雑すぎて内容が抜け落ちる」という続報が解消して
      いるか) を確認する。
      優先度: 中 / 所要時間: 10分 / 詳細: [PENDING.md](PENDING.md)「Task #160: スレッド要約のmapをメッセージ単位に固定」節

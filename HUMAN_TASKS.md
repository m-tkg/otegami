...
- [ ] **Task #195〜#198 (macOS レイアウト4件)** — macOS 側は実機ビルド
      (osascript+screencapture) で確認済み。iOS 側は #196 の未読のみ
      トグルが `#198`の footer toolbar と共に `#if os(macOS)` の外に
      変更が無いためビルドグリーンで担保しているが、実機/シミュレータ
      での目視再確認はできていない。詳細は [PENDING.md](PENDING.md)
      「Task #195〜#198」節。

...
- [ ] **スレッド要約 (■経緯/■現状) が最優先: 無関係な話 (ハルシネーション)
      が出なくなったか、かつ具体的・自然な口調・簡潔になったか** —
      複数往復のあるスレッド (できれば金額・日時・固有名詞など具体的な
      内容を含む4通以上のもの) を開き、ナビゲーションタイトル横の要約
      ボタンをタップして「n/m 通目を要約中…」に続き「仕上げ中…」という
      進捗表示が出ること、生成完了後の**■現状にそのスレッドと無関係な
      話題・数値・固有名詞が一切出ていないこと** (最優先確認事項)、
      ■経緯が数値・固有名詞・決定事項を落とさずに以前より少ない行数で
      簡潔にまとまっていること、かつ「この返信では〜が述べられている」
      のような説明調(メタ言及)の言い回しが出ていないこと、を確認する。
      短い(2〜3通の)スレッドや結論の出ていないスレッド、通知メールだけの
      スレッドでも同様に確認できるとなお良い。
      優先度: 最優先 / 所要時間: 15分 / 詳細: [PENDING.md](PENDING.md)「Task #160: スレッド要約のmapをメッセージ単位に固定・二重圧縮とメタ言及調を根治・仕上げパスで簡潔化・■現状のハルシネーション根治」節

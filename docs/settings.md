...
| 企業ロゴを表示 | `avatarSource.showCompanyLogo` | ON | アバター強化バッチ フェーズ3。連絡先の写真・Gravatar のどちらも見つからなかった差出人について、差出人ドメインの `https://<domain>/apple-touch-icon.png` → `/favicon.ico` の順に画像を取得する。**BIMI (DNS TXT レコードから SVG ロゴを取得) は実装していない** — システムリゾルバでの DNS TXT 取得に手頃な高レベル API が無く、SVG の安全なラスタライズも別途必要になるため、実装コストが見合わないと判断して見送った (`CompanyLogoAvatarResolver`のドキュメントコメント参照)。gmail.com/icloud.com/yahoo.co.jp 等の主要フリーメールドメイン (`FreeMailDomains`) は対象外 — ネットワークにすら問い合わせない。ドメイン単位 (メールアドレス単位ではない) でメモリ+ディスクキャッシュ (`Caches/AvatarCache/CompanyLogo/`)、TTL 30日、negative cache 含む。デコードできない画像データ (favicon.ico が真の ICO 形式の場合など) は「見つからなかった」として扱う。ドメイン名が接続先サーバーに送信される旨を設定画面に注記。 |

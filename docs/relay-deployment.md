...
> **Go 移植版 (Task #180)**: `server/otegami-relay-go/` に Swift 版と
> ワイヤ・ストレージ完全互換の Go 実装がある (同じ HTTP API・同じ SQLite
> スキーマ・同じ `RELAY_MASTER_KEY` 暗号化・同じ環境変数 — 既存の `.env`
> と DB がそのまま使える)。移植の動機は arm64 Docker イメージのビルドを
> QEMU なしで完結させること (x86_64 ランナーで約 2.5 時間 → 数十秒)。
> ビルド・互換性検証の詳細は `server/otegami-relay-go/README.md` 参照。
> 本番の切り替えはユーザー判断で行い、それまで本書の手順 (Swift 版) が正。

...
6. **信頼境界**: このリレーを運用する人は、登録された全アカウントの
   ...
   取り消せる。
7. **認証失敗時の自動停止**: IMAP ログインが連続 (既定3回) して失敗した
   watch は自動的に監視を停止する — パスワード変更後に古い資格情報で
   IMAP サーバーへの再試行を無限に繰り返し、アカウントロックアウトを
   誘発することを避けるため。`.oauth` watch は、refresh token 自体が
   失効/取り消し済み (`invalid_grant`) と分かった時点で即座に (3回を
   待たずに) 停止する。

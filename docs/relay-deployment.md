...
3. 実際のメール内容 (差出人・件名・本文プレビュー) は、プッシュを受信
   ...
   改めて IMAP に接続して取得する (アカウントの INBOX を差分同期して
   共有 DB へ永続化するのが主経路 — 「push 通知起点バックグラウンド
   受信」)。リレーは**既定では**メール内容を一度も扱わない — 預かるのは
   IMAP 資格情報 (パスワードまたは OAuth refresh token) のみで、それも
   暗号化して保存する (詳細は下記「セキュリティ設計」)。
   この取得処理は OS が拡張に与える時間予算 (約30秒) の内側で自前の
   表示デッドライン (5秒) を持ち、IMAP 同期や追加エンリッチが間に合わ
   ない場合は内容なしの汎用通知のまま表示を確定する — 通知自体が
   出ないことよりも内容が薄いことを許容する設計
   (`NotificationService.swift` のデッドライン定数参照)。
   `RELAY_CONTENT_PREVIEW` を有効にしたリレーと組み合わせると、この
   Extension は IMAP 接続を待たずに push payload 自体が運ぶ内容を先に
   表示できる — 詳細は下記「アプリ側の挙動 (NSE のリレー先読み統合)」
   参照。
...
### アプリ側の挙動 (NSE のリレー先読み統合)

`RELAY_CONTENT_PREVIEW=1` のリレーと組み合わせたとき、
`NotificationService` (`apps/Otegami/NotificationService/
NotificationService.swift`) の `enrich(payload:)` は次の順で処理する
(いずれもベストエフォート、失敗しても通知自体は生成され続ける):

1. push payload に同梱された `latestFromName`/`latestFromAddress`/
   `latestSubject` があれば、**ネットワークに一切触れずに**即座に通知の
   タイトル/本文へ反映する (通知文言の3トグルは通常どおり尊重する)。
   この時点で IMAP 接続もリレーへの HTTP リクエストも発生していない —
   拡張の5秒の表示デッドラインに間に合わなかった場合の保険としてまず
   これだけで意味のある文言を確定させる。
2. 本文プレビュー設定が on かつ payload に `latestUid` があれば、
   `GET /v1/messages?accountId=<id>&sinceUid=<latestUid-1>` (Bearer
   `deviceSecret`) を1回叩き、応答から `latestUid` と一致する1件の
   `bodyPreview` を取り出して通知本文へ追記する。リレーが 404 を返す
   場合 (機能 off、または該当 watch が無い — 上記「何が変わるか」の
   項目3のとおり区別できない) や、タイムアウト (5秒) を含むその他の
   失敗は無視して次へ進む — この HTTP リクエストも IMAP 接続を経由しない。
3. アカウントの資格情報を解決し、INBOX を差分同期して結果を共有 DB へ
   永続化する経路 (「push 通知起点バックグラウンド受信 Phase 1」) は
   無変更で実行する。これが唯一の IMAP 接続であり、唯一の DB 書き込み
   経路。同期が成功すれば、その結果 (実際に永続化されたメッセージ行) が
   最終的な通知文言・バッジ数の正としてステップ1/2の内容を上書きする。
4. 同期が成功し、かつステップ2でリレー由来の本文プレビューが取得できて
   いた場合、対象メッセージ行の `snippet` 列が**まだ `nil` のときだけ**
   そのプレビューを書き戻す (同期自体が本文プレビュー取得込みだった場合
   は既に `snippet` が入っているため、この書き戻しは無意味な二重書きに
   ならない)。**`bodyState` は絶対に変更しない** — リレーの
   `bodyPreview` は本文全体でも添付でもない切り詰め文字列でしかなく、
   `.fetched` 扱いにすると本文全体の取得 (`BodyFetcher`) が永久にスキップ
   されてしまう。

**旧リレー (これらのフィールドを送らない) や `RELAY_CONTENT_PREVIEW` が
off のリレーと組み合わせた場合は、ステップ1・2は何もせず**
(`accountId`/`uidNext` しか push payload に無いため)、ステップ3の
sync-first 経路とその失敗時の read-only IMAP フォールバックのみが動く
— これは Phase 3 以前と全く同じ挙動で、文言の後退は無い。取得試行/
成否は端末内の「プッシュ通知の診断」画面 (`PushDiagnosticsView`) の
`relayPreview` 段階に記録される。

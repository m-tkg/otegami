...
- 同様に、迷惑メールビュー (迷惑メールフォルダ/統合「迷惑メール」) を見
  ているときは「迷惑メールにする」が「迷惑メール解除」(`checkmark.shield`、
  受信トレイへ戻す) に切り替わる。メール本文画面のツールバーとコンテキ
  ストメニューも、そのスレッド/メールが迷惑メールにあるときは同じ入れ替
  えをする。
- **一覧のバッジと本文画面のスロットは別の述語を見る**。バッジ (行末尾の
  `archivebox`) は OR 集約 — スレッド内に1通でもアーカイブ済みがあれば
  付く (`ThreadQuery.isThreadArchived`)。一方、本文画面のツールバー/コン
  テキストメニュー/⌘E がアーカイブと「アーカイブ解除」のどちらとして振
  る舞うかは AND 集約 — **まだアーカイブできるメールが1通も残っていな
  い**ときだけ「アーカイブ解除」になる
  (`ThreadQuery.isThreadFullyArchived`)。迷惑メールも同じ組み合わせ
  (`isThreadJunk` / `isThreadFullyJunk`)。
  状態の可視化と「今押したら何が起きるか」は役割が違うので、混在スレッド
  では「バッジは付いているがスロットは『アーカイブ』のまま」という組み合
  わせになる — これは意図どおり。
  当初はスロット側も OR 集約だったが、「読み終えてアーカイブしたスレッド
  に返信が届く」という日常動作で必ず混在状態になり、スロットが「アーカイ
  ブ解除」に化けて新着をアーカイブできなくなる不具合になった (実機報告
  2026-08-27)。`MessageRemoval.commit(.unarchive)` はアーカイブ場所にいる
  行しか触らないため、押しても新着は素通りし、しかも何のエラーも出なかっ
  た。AND 集約の述語は `commit(.archive)` が実際に処理する行の有無とちょ
  うど一致するので、表示と挙動が構造的にずれない。
...
- **スワイプ操作** (1g): `.swipeActions` ではなく独自の `DragGesture` 実
  ...
  替わり、ロング (行幅の約50%) で長押しアクションの見た目に変わる。閾値
...
- 検索結果もスレッド表示設定 (アコーディオン/フラット) に従う。フラッ
  ...
  ということはない。
- 実機報告「削除したメールが検索に出てくる」: 削除・迷惑メール移送は
  ローカルではゴミ箱・迷惑メールボックスへの移送であり (`MessageRemoval
  .commit`)、メッセージ行は残る (意図された即時反映)。そのため検索
  (`SearchQuery`) は横断検索 (`.allAccounts`) スコープではゴミ箱・迷惑
  メール (`mailbox.role`) と非表示メールボックス (`mailbox.isHidden`)
  を対象から除外する。サーバーで検索 (`ServerSearchService`) も同様に
  除外する (Gmail は All Mail のみ検索するため元から対象外)。ユーザー
  が明示的にそのメールボックス自体を開いて検索する場合 (`.mailbox`ス
  コープ) は除外しない。

...
## Task #75: アカウント色の再調整 — ビビッド化

Task #72 で8色→20色に拡充した直後の実機フィードバック:「アカウントの
カラーは参考画像 (Spark のピッカー) と同じように、もっとはっきりした
色合いにして欲しい。いま、この各色は違いはあるけど、5段階の色の差は
小さいので」。Task #72 は意図的に「彩度・明度は既存8色と同じ上品な
抑えた家族に留め、参考画像のビビッドな配色は採用しない」という判断を
していた (前節参照) が、20点まで密に詰めた状態でその抑えた彩度を保つと
隣接する色相 (特に紫〜青の並び) の知覚差が足りない、というのが今回の
指摘 — 「上品さ」と「20色でも見分けが付くこと」が両立しなかった。

### `OtegamiAccountColor.swift` の20色すべてを高彩度トーンに作り直し

Task #72 では既存8色 (`teal`/`indigo`/`plum`/`amber`/`rose`/`sage`/
`slateBlue`/`coral`) の16進値を一切変更しない制約を置いていたが、
今回はその制約を外した — **20色全部**を、`PaletteColor`の宣言順
(色相の昇順、Task #72節の色相表と同じ角度) はそのままに、彩度・明度
だけを Spark の参考画像に近い高彩度ファミリーへ振り直した。色相角度
自体は変えていないので、どの色がどの色の隣かという並び (グリッド
ピッカーの表示順、円環距離の近似) は Task #72 から変わらない。

彩度・明度は全色一律ではなく色相帯ごとに調整した — 黄緑〜緑の帯
(`sage`/`green`/`emerald`) は赤・オレンジ系と同じ明度にすると人間の目
には眩しいネオン色に振れすぎるため明度をやや落とし、逆にダークモード
側の値は全色でライトモード側より彩度・明度を上げている (黒背景の上で
沈まず、ぱっと見て発色して見えるようにするため)。`DesignSystemCatalog
Renderer`のレンダリングで、Task #72 の初稿時に踏んだ「低彩度×黄緑帯が
オリーブ色に潰れる」落とし穴を今回も踏んでいないか確認しながら数値を
詰めた。

これは**既存アカウントの見た目に必ず影響する変更**である — Task #72
とは異なり、「既存の16進値を変えない」制約をこの回は意図的に外して
いる。ただし影響を受けるのは`PaletteColor`の`rawValue`(＝保存されて
いる`labelColorKey`文字列)ではなく、そのキーが指す`Color`の実際の値
だけなので、iCloud同期や保存済みデータの互換性には影響しない — 同じ
ケース名が指す色が新しいトーンに変わって表示されるだけで、これは
今回のフィードバックが求めている挙動そのもの。

### 確認

`DesignSystemCatalogRenderer`のlight/dark両出力と、
`scripts/verify-screen.sh account-edit`(Task #72で追加済みのシナリオ、
`APPEARANCE=light`/`dark`双方) のスクリーンショットで、20色+「自動」
の21スワッチすべてが隣接色と見分けられることを目視確認した — 特に
Task #72 時点で近く見えていた紫〜青の並びが、今回のビビッド化で
はっきり区別できるようになったことを確認済み。`apps/Otegami/
DesignSystemCatalog`の`DesignSystemTests`(`leastUsedColorKey(avoiding:)`
...
ダークモードで開くと、「会議のリンク」「日時」「ゲスト」のラベルと本文が
暗い背景にほぼ沈んで読めない。原因は判定の取りこぼし — Task #56/#80/#84
で拡張してきた「背景指定が無いメールでも、代表的な文字色の輝度を実測して
介入するかどうかを決める」ロジック
(`HTMLWebViewCoordinator.decideDarkInversion`/`representativeTextLuminance`)
は、文書順で見つかった**最初の6テキストノードだけ**を平均する設計だった。
Task #84 で追加したテスト用フィクスチャ (`AppEnvironment
.uitestFakeCalendarInviteHTML`) はこの6サンプルだけでも正しく判定できて
いたが、実機の実際のメールは本文冒頭にタイトル見出しや「〜さんが招待
しました」という色を明示指定しない前置きテキストが複数行あり、それらが
(このファイル自身の CSS リセット経由で明るく解決される `CanvasText`
として) 平均を押し上げて「介入不要」に誤って倒れていた。

### 修正: 文字数ベースの多数決を追加のフォールバックとして導入

`decideDarkInversion`の「背景が解決しない」分岐 (Task #56) に、既存の
6サンプル平均判定が「介入不要」に倒れたときだけ試す**追加の証拠**を足した
(`explicitDarkTextIsMajority`、`HTMLMessageView.swift`)。既存の判定・
閾値は一切変更していない (「維持」方針):

- `inner`(`#otegami-fit-inner`) 配下の**全テキストノード**を対象に、
  「祖先のいずれかがインライン`style`で`color`を明示指定しているか」
  (`nearestExplicitColorAncestor`) で仕分ける。継承のみ (色を一切指定
  していない) のテキストはカウントしない — プレーンテキスト風メール
  (Task #51 の32番フィクスチャ相当) を誤って白カード化しないための線引き。
  CSS の`<style>`ブロック経由のクラス指定までは追わない (インライン
  `style`のみ) — 実務上、招待/通知系のテンプレートはメールクライアント
  互換性のため文字色をほぼ例外なくインラインで指定するという現実的な
  割り切り (このファイル既存の`mailDeclaresOwnDarkModeSupport`等と同じ
  プラグマティックな方針)。
- 明示指定ありと判定されたテキストのうち、実測輝度 (`getComputedStyle`
  で解決した実際の色) が閾値 (**0.55**、`representativeTextLuminance`の
  0.5よりわずかに緩い — 白背景前提の中間グレー `#5f6368` 系まで拾うため)
  未満のものを「暗色」として文字数を積算する。
- 「暗色」の文字数が可視テキスト全体の文字数の**過半**を占めれば介入
  (ライト維持 = 白カード) と判定する。

### 検証

`make test`(既知の無関係flake — MessageBuilderTestsの日本語ラウンド
トリップ — 以外は緑)・`make ios`とも成功を確認。`scripts/verify-screen.sh`
の`html-0`〜`html-4`(既存5ケース: 白背景+濃色文字→白カード、色指定なし→
ダークネイティブのまま、自前ダーク対応→無変換、背景なし+`#444`文字→
白カード、`body`背景明示+透過ロゴ→白カード) をダークモードで再撮影し、
いずれも回帰がないことをスクリーンショットで目視確認した。既存の
`calendar-invite`シナリオ (Task #84 フィクスチャ) も引き続き白カードで
正しく読めることを確認した。

新規追加した `html-5`/`html-calendar-invite-realistic` シナリオ
(`AppEnvironment.uitestFakeHTMLMessageBodyCalendarInviteRealisticNotice`
— タイトル見出し+前置き文+CTAボタンの白文字で6サンプル平均判定をわざと
0.5超に固定しつつ、全体では`#5f6368`/`#3c4043`の明示色テキストが文字数で
過半を占める構造) で、この修正が無いと再現する取りこぼしそのものを
ダークモードでスクリーンショット確認し、白カードで「会議のリンク」
「日時」「ゲスト」を含む全文が読めることを確認した。

- **未検証**: 実機の実際の Google カレンダー招待メール本文そのもの
  (テンプレートの細部は送信元・時期で変わりうる) — このタスクのフィク
  スチャは実機報告の構造的特徴 (背景なし・ラベル`#5f6368`・本文冒頭の
  色未指定テキスト) を再現したものであり、実物と1バイト単位で同一では
  ない。**実機確認ポイント**: 実際に届いた Google カレンダー招待メール
  をダークモードで開き、「会議のリンク」「日時」「ゲスト」のラベルと
...
## Task #153: スレッド表示タイトル + スレッド全体 AI 要約

`ThreadDetailView`のナビゲーションタイトルを、グループ化/アコーディオン
表示 (`!isFlatModeEntry` — 実際のメッセージ数に関わらず、Task #64以来の
「真に1通のみのフラット表示かどうか」という既存の区別をそのまま使う)
では「スレッド」、真に1通のみのフラット表示では従来どおり「メール」に
変更した。「本文画面を開いたら常にスレッド全体を見ている」というアコー
ディオン表示の実態にタイトルを合わせる意図。

同じ条件 (`!isFlatModeEntry`) のときだけ、ナビゲーションタイトルの隣
(右上、`.toolbar`) にスレッド全体を要約する`"sparkles"`アイコンの
トグルボタンを新設した — アイコンは単一メッセージの「要約」ボタン
(`MessageDetailFooterToolbar.summarizeButton`、`MessageToolbarAction
.summarize`) と同じものを使い、見た目の一貫性を保っている。タップする
とスレッド内の全メッセージ(時系列順、本文未取得のものはベストエフォート
で取得を試みる)から「新規本文のみ」を抽出して1本のダイジェストにまとめ、
■経緯(時系列の経緯)/■現状(現在の状態)の2パート構造でAI要約するシートを
開く — 単一メッセージ要約 (■要約/■伝えたいこと/■アクションの3パート)
とは完全に独立した状態・呼び出し経路 (詳細は`docs/translation.md`の
「スレッド全体のAI要約 (Task #153)」節参照)。フラット表示 (1通のみ、
`isFlatModeEntry == true`) ではこのボタン自体を表示しない — 単一メッセ
ージの要約ボタンと機能が重複するため。

### 検証

`make test`(既知flakyの`MessageBuilderTests`日本語ラウンドトリップ以外
green)/`make mac` green。`scripts/verify-screen.sh thread-accordion`
(既存シナリオ、3通の架空フィクスチャスレッドを直接アコーディオン表示で
開く) のスクリーンショットで、ナビゲーションタイトルが「スレッド」に
なっていること・タイトル右にsparklesアイコンのボタンが表示されている
ことを目視確認した。ボタンをタップしてシートが開いた状態そのもの
(生成中/完成/失敗の各表示) のスクリーンショット確認は今回のセッションで
は見送った。その後 Task #160 でスレッド要約自体が map のみの単一
パイプラインへ全面刷新され (■経緯/■現状ラベルは廃止)、このシートの
出力構造も変わっている — 現在の確認ポイントは `docs/translation.md`の
Task #160 関連節、`PENDING.md`「Task #160」節参照。
...
**未検証だった上記の確認は Task #182 で解消**: 独立ウィンドウ
(`UpdateCheckView`/`UpdateCheckRequest`) は削除し、「Otegamiについて」
(About) パネルへ統合したアップデート確認/インストール UI に置き換えた。
残る未検証項目は `docs/release.md`「アプリ内アップデート (Task #182)」節、
`PENDING.md`「Task #182」節参照。
...
## Task #165: macOS 操作体系再設計

### 発端

実機フィードバック (2026-07-29、macOS 実機): 「mac 版において、下書きの
削除もできない。基本的に iOS の操作体系は完全に忘れて再設計してほしい」。
調査の結果、下書き削除が効かない直接原因は `DraftsView`(下書き一覧、
`SidebarView`/`MailScreenView` 双方から「下書き」行タップでシートとして
開く) が削除手段として SwiftUI 標準の `.swipeActions` しか持っていな
かったこと — **macOS の `List` にスワイプというジェスチャーは存在しない**
ため、この画面には macOS で削除する手段が文字通り何も無かった。

その後のユーザー指示で方針を明確化: 「操作体系に関しては、iOS は忘れて、
一般的な macOS アプリケーションとしての使用感で作成して欲しい。機能は
維持したまま。iOS 版ではスワイプだったものが、macOS 版では右クリックとか
別のボタンとか、そういうのでもいい」— 要件は**機能の網羅**(iOS 側にある
操作は必ず macOS でも別の形で到達可能にする)であって、UI の見た目を
iOS に似せることではない。

### 対応表 (iOS の操作 → macOS での置き場所)

iOS のスワイプ/長押し一括選択/FAB に載っている全機能を洗い出し、macOS 側
の到達経路をひとつずつ確認したもの。「変更なし」列は元々 macOS にも
既存の到達経路があった (今回追加していない) もの。

| iOS の操作 | 機能 | macOS の到達経路 | 状態 |
|---|---|---|---|
| スワイプ(短い/長い、割り当て設定あり) | アーカイブ | 行右クリック→「アーカイブ」/ スレッド詳細フッターツールバー / **⌘E**(新規) | 既存+新規 |
| スワイプ | 削除 | 行右クリック→「削除」/ フッターツールバー / **⌘⌫**(変更なし、既存) | 既存 |
| スワイプ | 既読/未読切替 | 行右クリック→「既読にする/未読にする」/ フッターツールバー「未読にする」/ **⇧⌘U**(新規、双方向切替) | 既存+新規 |
| スワイプ | ピン留め切替 | 行右クリック→「ピン留め/ピン留めを解除」/ フッターツールバー | 既存 |
| スワイプ | 迷惑メールにする | 行右クリック→「迷惑メールにする」/ フッターツールバー | 既存 |
| アーカイブビューでのスワイプ (「アーカイブ解除」) | アーカイブ解除 | 行右クリック→「アーカイブ解除」(既存、`isArchiveView`で自動切替) | 既存 |
| 長押し→一括選択→下部バー「既読に」 | 既読化(複数スレッド一括) | 単一選択のみ(下記「見送り」参照) | **見送り** |
| 長押し→一括選択→下部バー「移動」 | アーカイブ(複数一括) | 単一選択のみ | **見送り** |
| 長押し→一括選択→下部バー「削除」 | 削除(複数一括) | 単一選択のみ | **見送り** |
| フッターツールバーの返信/全員に返信メニュー | 返信・全員に返信 | 既存(共有コンポーネント、変更なし)/ 一覧行右クリックにも追加(スレッド最新メッセージ宛) / **⌘R**(既存)・**⇧⌘R**(新規) | 既存+新規 |
| フッターツールバーの転送ボタン | 転送 | 既存(共有コンポーネント)/ 一覧行右クリックにも追加 / **⇧⌘F**(新規、後述の付け替え) | 既存+新規 |
| スレッド詳細アコーディオンの各メッセージ行 | (個別メッセージへの操作 UI 自体が無かった) | **新規**: 各メッセージ行を右クリック→そのメッセージ宛の返信/全員に返信/転送 + スレッド全体への未読化/ピン/迷惑メール/アーカイブ/削除 | 新規 |
| 下書き一覧のスワイプ削除 | 下書きの削除 | **新規**: 行右クリック→「編集」「削除」/ ⌫キー / ツールバーのゴミ箱ボタン(いずれも単一選択) | 新規 |
| フローティング作成ボタン (FAB) | 新規作成 | サイドバーの「作成」ボタン + **⌘N**(既存、変更なし) | 既存 |
| フローティング検索ボタン (FAB) | 検索 | インライン `.searchable` 検索バー(既存) + **⌘F**(Task #165 で⇧⌘Fから移動、後述) | 既存(ショートカットのみ変更) |

**一括選択 (複数スレッド同時操作) を見送った理由**: 現状の
`MessageListView`(macOS)の行は素の `Button`(タップで即座に
`ThreadDetailView` を開く)で、`List(selection:)`によるネイティブな
複数選択には未対応。これを Set ベースの複数選択に作り直すには
`MessageListView.swift`(1900行超、`docs/ci.md`が警告する CI 型チェック
タイムアウトの既往歴あり)の行選択・タップ挙動・検索結果表示を横断する
書き換えが必要で、単発のタスクとしてはコストとリスクが見合わない —
ユーザー指示の「実装コストが大きければ単一選択の完全動作を優先し、複数
選択は範囲を報告して見送り可」に基づき明示的にスコープ外とした。単一
選択(右クリック/フッターツールバー/メニューコマンド)は全操作で完全に
動作する。

### 実装

- **メール一覧行の右クリックメニュー拡張**
  (`MessageListRow.contextMenuContent`): 返信/全員に返信/転送を先頭に
  追加(`ThreadSummary.latestMessage`宛)。`MessageListView`に
  `onReply`/`onReplyAll`/`onForward`(既定no-op)を追加し、macOS の
  `RootView.contentColumn`だけが`replySummary(_:replyAll:)`/
  `forwardSummary(_:)`(`RootView`の非`#if os(macOS)`ゲート済み — 理由は
  `handleThreadRemoved(_:)`と同じ、`contentColumn`自体は両プラットフォーム
  でコンパイルされるため)を渡す。iOS 側は既定値のままなので挙動は不変。
  アーカイブ/削除/既読未読/ピン/迷惑メールは Task #163 時点で既に
  `SwipeAction.allCases`を総なめするコンテキストメニューが実装済みだった
  (今回変更なし)。
- **スレッド詳細のメッセージ単位コンテキストメニュー**
  (`ThreadDetailView.ThreadMessageRow`): 新規追加。返信/全員に返信/転送は
  **そのメッセージ自身**の`messageId`宛(アコーディオンを展開しなくても、
  折りたたまれた行を右クリックすればそのメッセージに返信できる — footer
  ツールバー側の「常に展開中の1通が対象」より柔軟)。未読化/ピン/迷惑
  メール/アーカイブ/削除は**スレッド全体**に作用 — この アプリが既に
  確立している「削除・アーカイブ・ピン・既読状態はスレッド単位」という
  設計(`MessageListView`のフラットモードに関する doc comment)を踏襲し、
  `footerToolbar`が呼ぶのと同じ`archiveThread()`/`junkThread()`/
  `deleteThread()`/`togglePin()`/`markUnread()`をそのまま再利用(実装が
  二重化して挙動が乖離するリスクを避けた)。
- **下書きの削除 (最優先の実損)** (`DraftsView`): macOS 専用に
  `List(selection: $macSelection)`+行の`.tag(item.id)`+`.contextMenu`
  (編集/削除)+ツールバーのゴミ箱ボタン+`.onDeleteCommand`(⌫キー) を追加。
  いずれも既存の`pendingDeletion`確認アラート(iOS のスワイプ削除と共有)
  を起動するだけなので、ローカル下書き(`DraftMessageRecord`)・サーバー
  下書き(`DraftQuery.UnifiedRow.server`)のどちらの削除ロジックも変更して
  いない — 到達経路を追加しただけ。iOS は`List`(selectionなし)のままで
  挙動不変。
- **「メッセージ」メニュー (`OtegamiCommands`)**: ⌘R 返信(既存)・**⇧⌘R
  全員に返信**(新規)・**⇧⌘F 転送**(新規)・**⌘E アーカイブ**(新規)・
  **⇧⌘U 既読/未読切替**(新規)・⌘⌫ 削除(既存)。`RootView`に
  `archiveSelectedThread()`/`toggleReadSelectedThread()`/
  `forwardSelectedThread()`を追加、`replyToSelectedThread(replyAll:)`に
  `replyAll`引数を追加。いずれも選択中スレッド(または`selectedMessageId`
  が立っているフラットモード時はその1通)に作用し、
  `ThreadDetailView.threadSummary(threadId:singleMessageId:accountId:db:)`
  を`private`から同一モジュール内アクセス可能に緩和して再利用 — 3つ目の
  重複実装を避けた。`archiveSelectedThread()`は`MessageRemoval.commit`
  経由なので Task #163 のピン留めアーカイブ禁止ガードも自動的に効く
  (ブロックされた場合はこの経路にはトーストが無く静かに no-op — 既存の
  `deleteSelectedThread()`と同じ「ベストエフォート」方針を踏襲)。
- **⌫ の単独キー割り当てを避けた理由 (「E アーカイブ」の解釈)**:
  当初のユーザー指示は「⌫ 削除、E アーカイブ (Mail.app 準拠)」という
  修飾キー無しの表記だった。実際に検証したところ、SwiftUI/AppKit の
  メニューキー等価はテキストフィールドのフォーカスと無関係にアプリ全体で
  先取りされる — 修飾キー無しの文字キーをメニューへ割り当てると、件名欄・
  本文・検索欄に該当の1文字を打つたびにメニューコマンドが発火し、通常の
  タイピングを破壊する(Apple の HIG がこの理由で単独文字キーの割り当てを
  避けるよう定めている)。削除は既存の ⌘⌫ (実 Mail.app と同じ) をそのまま
  維持し、アーカイブは ⌘E (Airmail/Spark 等、複数のネイティブ Mac メール
  クライアントが実際に使っている組み合わせ) にした。実クリック検証で
  件名欄・検索欄への通常のタイピングに副作用が無いことを確認済み(後述)。
- **⇧⌘F の付け替え**: このメニューは元々 ⇧⌘F を「検索フィールドに
  フォーカス」に割り当てていた(M10)。ユーザー指示は転送にこの組み合わせ
  (実 Mail.app 準拠)を明示的に求めており、同じキー組み合わせを2つの
  コマンドで奪い合うことはできないため、検索の方を実 Mail.app 自身の
  検索ショートカットである ⌘F へ移動した(`OtegamiCommands`/
  `MessageListView.isSearchFieldFocused`のdoc comment、
  `README.md`/`docs/qa-findings.md`の記述も追随して更新)。

### 見送った/対象外にしたもの

- 一括選択(上記参照) — 明示的にスコープ外、単一選択で全操作カバー。
- 「移動」(iOS の一括操作の「移動」ボタンは実質アーカイブへの移動) —
  macOS 側は既存のアーカイブ操作(行右クリック/フッターツールバー/⌘E)が
  同じ機能を提供するので、別名のボタンを追加していない。
- 設定画面・作成画面(`ComposerView`/`RichText*`) — 並行して他エージェント
  が作業中のため意図的に触れていない(下書きの「編集」は既存の
  `onOpenDraft`コールバック経由で既存の作成画面を開くだけ)。

### 検証

`make test`(既存のOtegamiKit単体テスト群、Task #165固有の新規テストは
無し — 今回の変更はSwiftUIのビュー配線とメニューコマンドが中心で、既存の
`MessageRemoval`/`ThreadQuery`等の下位ロジックは呼び出し方を追加しただけ
で中身を変えていない)/ `make mac` / `make ios` すべて green。

`make mac`で`RootView.navigationViewWithFocusedValues`の
`focusedSceneValue`チェーンが4件→9件に増えたところで`docs/ci.md`が警告
する「CI型チェックタイムアウト」の症状(「the compiler is unable to
type-check this expression in reasonable time」)をローカルで実際に再現
— `navigationViewWithPrimaryFocusedValues`という別プロパティに5件/4件へ
分割して解消(そのdoc comment参照)。

macOS 実機での実クリック検証は`docs/verify.md`の「macOS 検証 (M10)」節
(CGEvent/AX driverベース) の手順で着手した — scratchpad に `driver.swift`
(CGEvent クリック/右クリック/キー入力に加え、AXUIElementPerformAction
経由の `press`/`showMenu`(=`AXShowMenu`、右クリック相当をクリック座標に
頼らず起動)、メニューバー項目を直接押す `menuclick`/`menuinspect` を
今回追加) を用意し、`OTEGAMI_UITEST_INSERT_FAKE_GMAIL_ACCOUNT=1`で
アプリを直接起動、下書き一覧(`DraftsView`)を開くところまでは確認できた。

**未完了 (このマシンが共有デスクトップのため中断)**: 下書き行を右クリック
する手前で `osascript` によるフロントモストアプリ確認を挟んだところ、
起動直後は `Otegami` だったフロントモストプロセスが、ごく短時間のうちに
`Google Chrome` に変わっていた — このマシンは隔離された CI サンドボックス
ではなく、ユーザー本人や他の並行エージェントが同時に同じ物理ディスプレイ
を使っている可能性がある共有環境だと判明した。`CGEvent(...).post(tap:
.cghidEventTap)`によるクリック/右クリックはグローバル HID タップへの
注入で、対象 PID を指定できない (実際のマウス操作と同じくその瞬間
フォアグラウンドにあるウィンドウへ届く) ため、フォーカスが移った後に
発行したクリックは Otegami ではなく他のウィンドウ (無関係な作業) に
届いていた可能性がある。`docs/verify.md`/`.claude/skills/verify/
SKILL.md`が明示する「シミュレータで詰まったら粘らず人間に引き継ぐ」
方針をこの macOS 版の等価物として適用し、それ以上の座標クリックは
即座に中断・起動したテスト用プロセスは終了した — 実際に誤って操作した
形跡は確認していないが (直前のクリックは1回のみで、宛先も宛先不明の
ブックマーク/タブ領域と見られる)、意図的にそれ以上のクリックは行って
いない。

**確認済み (ビルドのみ)**: `make test`(既存の失敗1件——
`MailTransportMailCoreTests`の日本語RFC822往復テスト——はこのタスクが
一切触れていないファイル由来で無関係、他エージェントの並行作業による
既存不具合と判断) / `make mac` / `make ios` すべて green。

**ユーザーに引き継ぐ実機確認ポイント** (このマシン、または別の実機で
Otegami.app を通常起動して確認):
1. 「下書き」を開き、下書き行を右クリック → 「編集」「削除」の
   コンテキストメニューが出ること。「削除」→確認アラート→一覧から消える
   こと。
2. 下書き行をクリックして選択 → ⌫キーで同じ確認アラートが出て削除できる
   こと。
3. 下書き行を選択した状態でツールバーのゴミ箱アイコンが有効化され、
   クリックで同じ削除フローが起動すること (未選択時は無効)。
4. メール一覧の任意の行を右クリック → 返信/全員に返信/転送が先頭に、
   その下にアーカイブ/削除/既読・未読切替/迷惑メール/ピン留めが並ぶこと。
5. スレッド詳細画面で、折りたたまれた (未展開の) メッセージ行を右クリック
   → そのメッセージ宛の返信/全員に返信/転送 + スレッド全体への操作が
   出ること。
6. スレッドを開いた状態で「メッセージ」メニュー (メニューバー) を確認 —
   返信 (⌘R)・全員に返信 (⇧⌘R)・転送 (⇧⌘F)・アーカイブ (⌘E)・既読/未読を
   切り替え (⇧⌘U)・削除 (⌘⌫) が有効。スレッド未選択時はすべて無効化
   (グレーアウト) されていること。
7. 件名欄・検索欄に "e" を含む文字列を普通にタイプできること (⌘E の
...
## 実機報告: 「ツールバーをカスタマイズ」の位置が安定しない (Task #188)

実機報告「メールビューで、『ツールバーをカスタマイズ』の位置が一番下に
なることがある。たぶんツールアイコンが多くてスクロールする状態だと
発生する」の調査・修正記録。

### 再現条件として事実だったこと

「メニューが上向きに開くと項目の並びが反転する」という当初の仮説
(タスク側の「有力な筋」) は、**このセッションでは実機/シミュレータで
確認できなかった** — 切り分け用の一時 XCUITest (「その他」メニューを
実タップして開いた状態を`XCTAttachment`でスクリーンショット、非表示
アクション0件/7件 (既定) の2構成で比較する設計) を用意したが、
`docs/verify.md`「シミュレータ検証の既知の不調」節・既知不調#6 (Task
#173/#184/#185) と同じ症状 (`xcodebuild test`が無応答のまま進捗が
一切出ない) に阻まれ、実行が完了しなかった。このセッション中は他
エージェント (#189) も同じ共有ツリーでビルド・編集を並行して行って
おり、既知不調#6が挙げる「並行ビルド/`simctl`デーモンの競合」という
状況とも矛盾しない。ユーザー指示「シミュレータで粘らない」に従い、
1回の試行で打ち切った (`docs/verify.md`の同節に追記あり)。

一方、コードを読んだ限りで確認できた事実は次のとおり:

- Task #113 (1) で既に対処済みだった原因 (「削除」項目の`role:
  .destructive`が iOS の`Menu`によって自動的にメニュー最後尾へ
  移動させられる) は、現在のコードには存在しない —
  `MessageDetailFooterToolbar.swift`内で`role: .destructive`を
  使っているのは、フッターツールバーに直接表示されている`deleteButton`
  (どの`Menu`にも属さない単独の`Button`) だけで、「その他」メニュー内の
  「削除」(`hiddenActionMenuItem(for:)`の`.delete`ケース) は既に
  `.tint(OtegamiColor.destructive)`に変更済み。つまり Task #113 (1) と
  **全く同じ原因の再発ではない**。
- 他に`.destructive`相当の属性 (自動並び替えの対象になり得るもの) を
  持つ項目は、このメニュー内には見当たらなかった。

### どう安定させたか

原因の具体的なメカニズム (上向きに開く/スクロールする、のどちらが実際に
描画順へ影響するのか) を特定できなかったため、**メカニズムを特定せずに
問題のクラスごと消す**方向で構造を変えた
(`MessageDetailFooterToolbar.moreMenuButton`/`hiddenActionsMenu`のdoc
comment参照):

- 表示オフにしたアクション (0〜13件、可変長 — アイコンを多く隠すほど
  長くなり、報告の言う「アイコンが多くてスクロールする状態」になり
  得るのはここ) を、`hiddenActionsMenu`という**別階層のサブメニュー**
  にまとめて格納するようにした (ラベル「メールの操作」)。
- 「ツールバーをカスタマイズ」は、その可変長サブメニューとは独立に、
  トップレベルの「その他」`Menu`に常に固定1件だけ並ぶ項目のままにした。
- この結果、トップレベルの「その他」メニュー自体は常に高々2件
  (`hiddenActionsMenu`が1件＋「ツールバーをカスタマイズ」が1件、
  非表示アクションが1つも無ければ後者だけの1件) — 表示オフの
  アクション数が0件でも13件でも、トップレベルの項目数・並びは
  変わらない。可変長・スクロール対象になり得るのは`hiddenActionsMenu`
  という別の`Menu`インスタンスの中身だけで、その内部がどう描画される
  かは親メニューの項目数にも「ツールバーをカスタマイズ」の位置にも
  影響しない。

トレードオフとして、表示オフにした個々のアクション (例: 「削除」) へは
「その他」→「メールの操作」→対象、と1タップ増える。既定でツールバーに
直接表示される6アクション (返信/転送/検索/情報/要約/翻訳) には影響
しない。

### 検証

`make test`/`make mac`/`make ios`/`make check-localization`すべて
green。新規文言「メールの操作」は`scripts/generate-localizable.py`に
ja/en を追加してから`Localizable.xcstrings`を再生成した。
...
## Task #198: macOS のメールビューのツールバーアイコンをコンパクトにする

**実機フィードバック**: 「mac 版において、メールビューでのアイコンが
大きすぎるしバランスがおかしい。もっとコンパクトなアイコンにして」。

### 大きさの出どころを特定 (決め打ちで変えない)

`MessageDetailFooterToolbar`(iOS/macOS 共用) を実機で screenshot して
確認したところ、`toolbarIcon(_:title:systemImage:tint:)`自体の数値
(アイコン`.font(.system(size: 18))`、`.otegamiMinimumTappable()`の
44×44、外側の`.padding(OtegamiSpacing.sm)`) は iOS と全く同じで、以前
から変わっていなかった。実機で目に見えていた「大きな角丸の濃い背景」は
コード上どこにも明示的に書かれておらず、**このツールバーのどの
`Button`/`Menu`にも`.buttonStyle`/`.menuStyle`を一切指定していなかった
ため、macOS の既定 (`.automatic`) が bordered 風のスタイル (アイコン
ごとに濃い角丸の背景を描く) を割り当てていた**ことが原因だった —
明示サイズでもパディングでもフォントサイズでもなく、**ボタンスタイル**
が出どころだったと判断した。iOS はこの既定スタイルがこの見た目の背景を
描かない (Liquid Glass の場合はタップ時のみ) ため、この UI 崩れは
macOS だけで顕在化していた。

### 実装

`body`の`ViewThatFits`に`#if os(macOS)`で`.buttonStyle(.plain)`+
`.menuStyle(.borderlessButton)`を1箇所だけ追加し、環境経由で配下の
全`Button`/`Menu`(`replyMenuButton`/`moreMenuButton`は`Menu`、残りは
`Button`) へカスケードさせた。`.buttonStyle`は`Menu`には効かないため
別途`.menuStyle(.borderlessButton)`(macOS 標準の「枠なしのプルダウン」
スタイル) が必要だった。これでこのアプリの他の場所 (`AccountFilterChip`
/`SearchTopBar`等) と同じ「背景なし、素のアイコン」というボーダーレスな
見た目に揃った。**iOS は`#if os(macOS)`の外なので無条件に無変更**。

アイコンの明示サイズ (18pt)・タップ領域 (`.otegamiMinimumTappable()`の
44×44)・外側パディングはあえて変更していない — ボタンスタイルの修正
だけで実機スクリーンショット上「大きすぎる」という見た目が解消された
ため (後述の検証参照)、追加でこれらの数値まで変えるのは過剰修正だと
判断した。44×44 のタップ領域自体は背景を描かなくなった今は完全に
不可視で、クリックのしやすさに寄与するだけで見た目には影響しない。

### 返信/「その他」の下向き矢印 (幅がまちまちな点) の扱い

`返信`(`Menu`、返信/全員に返信の2択) と「その他」(`Menu`、隠しアクション
+ツールバーカスタマイズ) だけ、コンテンツ自体にmacOS標準のプルダウン
矢印「⌄」が付き、他のアイコンより横幅が広くなる。**この幅の違いは
そのまま残すと判断した** — 矢印はこのボタンが単なるボタンではなく
メニューを開くことを示す標準的なアフォーダンスで、`Menu`である以上
`.menuStyle(.borderlessButton)`でも消えない (消すべきでもない、「これは
選択肢がある」という手がかりを失う)。すべてのアイコンを均等幅
(`.frame(width:)`固定) に揃える案も検討したが、内容に応じた自然な幅は
macOS のツールバーでは珍しくなく (例: Apple Mail自体のツールバーも
アイコンごとに幅が揃っていない)、任意の固定幅を決め打ちで導入する
方がむしろ「決め打ちで数値を変えない」という指示に反すると判断した。

### 検証

`make mac`でビルドし実際に起動、実在アカウントのメッセージを開いて
`osascript`+`screencapture`で修正前後を比較: 修正前は9アイコンそれぞれが
濃い角丸の背景付きで大きく間延びして見えていたのに対し、修正後は背景が
消えて素のアイコンだけが並ぶ、コンパクトで一貫した見た目になったことを
確認した。`make ios`もビルドグリーンを確認 (`#if os(macOS)`の外に一切
触れていないため、コード上 iOS の`toolbarIcon`/`toolbarAIIcon`呼び出しは
無変更)。**iOS 実機/シミュレータでの見た目の再確認は未実施** — 既存の
`docs/verify.md`記載のシミュレータ既知不調があり、かつコード変更が
`#if os(macOS)`ブロックの外に一切無い (ビルドグリーンで担保) ため、
...
## Task #200: Composer 宛先サジェスト

「Spark では、メール作成画面でメールアドレス入力時にサジェストが出る。
実装して欲しい」への対応。`ComposerView`の宛先(To/Cc/Bcc)は元々
「カンマ区切りのプレーンテキスト、トークン化はlater」という設計
(`ComposerView`冒頭のdoc comment) のままで、今回もそれ自体は変えて
いない — 入力中の1トークン(直近のカンマから後ろの部分文字列)に対して
候補を出し、選ぶとそのトークンを`"Name <address>, "`に置き換えて次の
入力へ続けられるようにした。

### 候補の出どころ: 過去のやり取りのみ、連絡先は混ぜない

`RecipientHistoryQuery`(`packages/OtegamiKit/Sources/OtegamiStore/`)が
`message`テーブルの`from`/`to`/`cc`から候補を作る。端末の連絡先
(`ContactPhotoResolver`が既にアバター用に使っている`CNContactStore`)は
**意図的に混ぜていない**:

1. 過去のやり取りだけなら**許可が一切要らない** — 「連絡先の許可が無い/
   拒否されていても動く」という要件を、許可の有無を気にする分岐を一切
   書かずに満たせる。
2. これから書こうとしている相手として、メール履歴が無い連絡先より
   メール履歴がある相手の方が関連度の高いシグナルになりやすい。
3. 2ソースを混ぜると優先順位・重複解決のポリシーが増え、検証すべき
   組み合わせが倍になる — 初版でその複雑さに見合う効果があるか疑わしい
   と判断した。

将来、既に許可済みのContactsデータを「メール履歴に無い相手を補う低優先
ソース」として足すことは、この設計を壊さずに追加できる(`RecipientHistorySource`
の差し替え/追加ポイントとして`RecipientSuggestionSource`を用意してある)。

### 並び順: 頻度 (log2) + 新しさ (180日で線形減衰) の加算

`RecipientSuggestionEngine`(`packages/OtegamiKit/Sources/OtegamiCore/`、
DB非依存の純粋関数)が担う。同じ相手への複数回のやり取りは
`address`(大小文字を無視)で1件に集約し、

```
score = log2(count + 1) + max(0, 1 - daysSinceLatest / 180)
```

- **頻度**を対数にしたのは、1人のやり取り回数がどれだけ多くても
  (メーリングリストのように毎回`to`/`cc`に載る相手など)、それだけで
  他の相手を完全に埋もれさせないため — 追加1通あたりの重みは
  逓減する。
- **新しさ**は180日で線形に1→0へ減衰し、それ以降は0で下げ止まる
  (マイナスにはしない) — 「2年前に50回やり取りしたきり」の相手が
  「今週2回やり取りした」相手に自動的に負けつつも、頻度側のスコアは
  失わない(新しさの加点が無くなるだけ)設計。
- 両者を**加算**にしたのは、「1回だけ今日やり取りした相手」と
  「10回、2ヶ月前にやり取りした相手」のどちらも状況次第で妥当な1位に
  なり得るため — 掛け算にすると片方が0に近いだけでもう片方の効果まで
  ほぼ消えてしまう。実際の比較例・境界ケースは
  `RecipientSuggestionEngineTests`の
  `recencyCanOutweighOldFrequency`/`frequencyCanOutweighSingleRecentMessage`
  参照。

表示名が同じアドレスで複数種類ある場合(表示名を後から変えた相手、
"Tanaka Taro" → "田中太郎" など)は**最新の出現の表示名**を採用する
(頻度・新しさの集計自体は表示名に関わらず全出現をまとめる)。

### 絞り込み: 表示名・アドレスどちらも部分一致 (大小文字/発音区別なし)

`String.range(of:options:[.caseInsensitive, .diacriticInsensitive])`
を表示名・アドレス両方に対して行う。「たなか」で日本語表示名にヒット
させたい、という要件は「表示名の文字列にたまたま『たなか』という
部分文字列が含まれていれば単純な部分一致で引っかかる」という素直な
実装で満たしている(読み仮名とのマッチングのような形態素解析は
やっていない — 表示名自体がひらがな/カタカナを含むケースをカバーする
だけで十分と判断した)。

### 重複除去

`RecipientHistoryQuery`はアドレスをまとめずに生の出現をそのまま返し、
`RecipientSuggestionEngine`側でアドレスを小文字化したキーで集約する
(表示用のアドレス文字列自体は最新出現の元の大小文字を保つ — 集約キー
にだけ小文字化を使い、UIに出す文字列を強制的に小文字化しない)。

### 性能: まず実測してから決めた `scanLimit`

「入力のたびに検索が走る」の“検索”を2段に分解して考えた:

- **DBスキャン+デコード**(`RecipientHistoryQuery.occurrences`)は
  作成画面を開いた瞬間に**1回だけ**(`RecipientSuggestionSource`が
  コンポーズセッションを跨いで結果をキャッシュする)。
- **ランキング・絞り込み**(`RecipientSuggestionEngine.suggestions`)が
  キーストロークのたびに走る。

5万通・送受信相手300人規模の合成データでの実測 (詳細・全数値は
`docs/performance.md`「Task #200」節):
`scanLimit: 3000`で**83.5ms**(1回だけ発生)、`suggestions()`は
3,600件のoccurrencesに対して**3.3ms**(キーストロークごと)。どちらも
体感を損なわない範囲に収まっていることを確認したため、追加のキャッシュ
やデバウンスなどは導入していない (「先回りして複雑にしない」方針)。
`RecipientSuggestionSource.defaultScanLimit = 3000`という具体的な数値も
この実測に基づく。

### レイアウト: ポップオーバーではなくインライン展開

候補は`.popover`のような別プレゼンテーションではなく、フィールド直下に
行を追加する形でインライン展開する(`RecipientInputField`、
`apps/Otegami/Sources/Features/Composer/`)。理由は2つ:

1. macOSの`addressSection`はForm/Sectionの自動ラベル配置(Task #164の
   「見出しとラベルの二重表示」修正)に乗っている — `TextField`を独自の
   コンテナでラップするとFormがラベル行として認識しなくなるおそれが
   あるため、`LabeledContent`(Formの自動配置自体が内部で使っている、
   公開ドキュメント化されたプリミティブ)を明示的に使い、その
   トレーリングコンテンツの中でフィールドと候補行を縦に並べた。
2. iOSで`.popover`を使うと、候補の`Button`をタップした瞬間に別
   プレゼンテーションへフォーカスが移り`TextField`のキーボードが
   閉じる/フォーカスが外れるリスクがある — 「候補を選ぶと宛先が確定し、
   続けて次の宛先を入力できる」という要件と相性が悪い。インライン展開
   なら同じビュー階層のままなので、この心配が要らない。

macOS は上下矢印キー+Enter (`.onKeyPress`)、iOS はタップで選択する
(仕様どおり)。

### 検証

`make test`(`RecipientSuggestionEngineTests`/`RecipientHistoryQueryTests`
を含む)green。macOSは`make mac`でビルドし実際に作成画面を開いて候補が

...
### タグの付け方とバージョン比較のバグ (Task #214)

**症状 (ユーザー報告)**: `v1.2.0-beta13` まで GitHub Release が出ている
状態でアプリ内アップデートを確認すると、`v1.2.0-beta9` が「最新」だと
判定されてしまう。

**原因**: `SemanticVersion.swift`のプレリリース識別子比較自体は
[SemVer 2.0.0 §11](https://semver.org/#spec-item-11)の規格どおり正しく
実装されていた。問題は**タグの付け方**だった —
このプロジェクトの実タグは `v1.2.0-beta9`/`v1.2.0-beta13` のように
ドット無し (`git tag -l "v1.2.0-beta*"` で確認できる、`beta` と数字が
くっついた1つの識別子)。規格どおりのプレリリース比較では、`beta9`と
`beta13`はどちらも「1つの英数字識別子」として ASCII 順で比較される —
`"beta13" < "beta9"`(先頭から見て `1` < `9` の位置で決まる) となり、
**beta13 が beta9 より古いと判定されてしまう**。しかも今後
`beta.13`(ドット区切り)に書き方を変えるだけでは直らない —
既存の`beta9`タグと比較する際、`["beta","9"]` vs `["beta","13"]`の
最初の識別子`"beta" < "beta9"`でやはり`beta9`が勝ってしまう
(既存タグが残っている限り、比較側を直さない限りどちらの書き方でも
再発する)。

**修正**: `SemanticVersion.init?(parsing:)`が、プレリリース識別子を
パースする際に「英数字 + 末尾の数字」を分解するようにした
(`splittingTrailingDigits`) — `beta9`は`["beta", "9"]`に、`beta13`は
`["beta", "13"]`になる。これは`beta.13`と書いたときに得られる識別子
列と同じなので、**ドット無し (`beta9`) とドット区切り (`beta.9`) の
どちらの表記でも同じ比較結果になる**。数値部分は数値として比較される
ため`9 < 13`が正しく成立する。これは SemVer §11 の規格からの意図的な
逸脱 — 詳細な理由は`SemanticVersion.swift`の該当コメント、テストは
`SemanticVersionTests.swift`の「Task #214」節を参照。

**今後のタグの付け方**: 上記の修正で `betaN`(ドット無し) と
`beta.N`(ドット区切り) の両方が正しく比較されるようになったため、
**タグの付け方を変える必要はない** — 引き続き `v1.2.0-beta9`
のような従来どおりの書き方で問題ない。既存タグ (`v1.2.0-beta` 〜
`v1.2.0-beta13`) との後方互換性を保ちたければなおさらこのままが安全
(書き方を混在させても比較結果は変わらないが、変える理由も無い)。

**影響範囲**: macOS のアプリ内アップデートチェック
(`UpdateAvailability.check`)のみ。iOS の TestFlight 配布・OTA 配信は
別経路の比較を行わないため影響を受けない。

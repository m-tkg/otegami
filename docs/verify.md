...
3. **アバター解決の連絡先権限ダイアログが非決定的なタイミングで出て
   ...
   **2026-07-31 追記 (Task #189)**: `settings`/`general-settings`シナリオ
   で再度同じハングを2回確認 (この時も他エージェント (#188) の
   xcodebuildが並行実行中だった) — 単発の不調ではなく、他ジョブとの
   並行実行時に再現しやすい既知の不調として扱ってよさそうだという傾向が
   強まった。対策 (根本原因の特定含む) は未着手のまま — 発生したら
   このセッションと同じく2-3回でリトライを打ち切り、`PENDING.md`に
   未検証と明記して出荷する運用を継続する。
...
6. **(Task #173、原因未確定・要再現確認) `scripts/verify-screen.sh` の
   ...
   **2026-07-31 追記 (Task #188、同日)**: `verify-screen.sh`経由ではなく
   自前の`xcodebuild test -only-testing:...`(「その他」メニューを実タップ
   して開いた状態をスクリーンショットで残す、切り分け用の一時テスト)
   でも同じ症状に遭遇した — `xcrun simctl io booted screenshot`をポーリング
   する間ずっと出力ファイルが0バイトのまま、`ps aux`にも`xcodebuild`
   プロセスが見当たらなかった。このセッションでも並行して他エージェント
   (#189 が`scripts/generate-localizable.py`/設定画面を編集中) が同じ
   共有ツリーで作業していたため、Task #185 の「`xcodebuild`単体のロック
   だけでなく`simctl`デーモン自体の競合」という仮説とも矛盾しない。
   ユーザー指示「シミュレータで粘らない」に従い、この回では実機/
   シミュレータでの実際のメニュー開閉確認を待たずに打ち切った — 詳細・
   結果は`docs/design-system.md`「実機報告:「ツールバーをカスタマイズ」
   の位置が安定しない (Task #188)」節、実機での確認ポイントは
   `PENDING.md`「Task #188」節参照。
